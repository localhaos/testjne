...
#include <sys/prctl.h>
#include <sys/uio.h> // For readlink
...
// Blacklist for anti-detection (but we allow Frida to load)
static std::vector<std::string> g_blacklist = {
    ...
    "magisk",
    "frida-server"
    // NOTE: "frida" is NOT in the blacklist to allow it to load
};
...
void wipe_elf_header(void* base_addr) {
    ...
    memset(base_addr, 0, 64); // Wipe ELF header (first 64 bytes)
    change_memory_protection(reinterpret_cast<void*>(page_start), PROT_READ | PROT_EXEC);
}

// ------------------------------------------------------------
// INLINE HOOK (ARM64/ARM/x86)
// ------------------------------------------------------------
typedef struct {
...
int InlineHook_enable(InlineHook* hook) {
    ...
    // Allocate executable memory for the target address
...
// --- Block blacklisted files (but allow Frida) ---
...
char* my_fgets(char* s, int size, FILE* stream) {
    ...
    if (result && contains_blacklist(result)) {
        // Recursively call to skip blacklisted lines
...
int my_sigaction(int signum, const struct sigaction* act, struct sigaction* oldact) {
    if (signum == SIGSEGV || signum == SIGBUS || signum == SIGABRT) {
        // Ignore crash signals to prevent tombstone
...
void* load_from_assets_stealth(JNIEnv* env, jobject assetMgr) {
    ...
    while ((fileName = AAssetDir_getNextFileName(dir)) != nullptr) {
        ...
        if (buffer[0] == 0x7F && buffer[1] == 'E' && buffer[2] == 'L' && buffer[3] == 'F') {
            ...
            // Wipe ELF header to hide from memory scanners
            struct link_map* map;
            ...
            munmap(addr, size);
            close(fd);
            break; // Load only the first valid library
        }
        AAsset_close(asset);
    }
    AAssetDir_close(dir);
...
extern "C" JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM* vm, void* reserved) {
    ...
    {
        // Hook syscall (allow Frida but block other ptrace)
        ...
        // Hook prctl (block PR_SET_DUMPABLE)
        ...
        // Hook openat (block blacklisted files)
        ...
        // Hook fgets (block blacklisted strings)
        ...
        // Hook readlink (fake /proc/self/status)
        ...
        // Hook sigaction (block tombstone)
    ...
    LOGI("StealthHook loaded successfully!");
